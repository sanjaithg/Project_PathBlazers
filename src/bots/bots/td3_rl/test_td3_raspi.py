...
class TD3Tester(Node):
    def __init__(self):
        ...
        self.num_goals = len(self.custom_goals) 
        
        self.get_logger().info(f"Testing {self.num_goals} custom goals")
    ...
    def _set_custom_goal(self):
        """Manually sets the goal in the environment and logs it."""
    ...
    def _timer_callback(self):
        ...
        # --- MODIFIED: Clip speed to 0.4 m/s ---
        # Clipping linear x, linear y, and angular z
        max_speed = 0.1
        action = np.clip(action, -max_speed, max_speed)
        
        a_in = [float(action[0]), float(action[1]), float(action[2])]
        ...
        if done:
            ...
            if target_reached:
                ...
                self.get_logger().info(f"✓ GOAL {self.current_goal_idx + 1} REACHED in {steps_taken} steps ({time_taken:.1f}s)")
            elif reward < -100:  
                self.collisions += 1
                self.get_logger().warn(f"✗ COLLISION on goal {self.current_goal_idx + 1} after {steps_taken} steps")
            else:
                self.timeouts += 1
                self.get_logger().warn(f"✗ TIMEOUT on goal {self.current_goal_idx + 1} after {steps_taken} steps")
